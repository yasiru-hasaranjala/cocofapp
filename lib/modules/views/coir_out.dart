...
class CoirOutputs extends StatefulWidget {
  const CoirOutputs({super.key});

  @override
  State<CoirOutputs> createState() => _CoirOutputsState();
}

class _CoirOutputsState extends State<CoirOutputs> {
        ...
        child: Padding(
          ...
          child: Column(
            ...
            children: [
              ...
              StreamBuilder(
                    stream: ref.child('coir_out').onValue,
                    builder: (context, snap) {
                      if (snap.hasError) {
                        return const Text('Something went wrong');
                      }
                      ...
                      if(dropdVal != null)
                      {
                        items = items.where((i) => i['buyr'] == dropdVal).toList();
                      }
                      if(dropdV != null)
                      {
                    ...
                    return DataTable(
                      ...
                      columns: const[
                          ...
                          DataColumn(label: Center(child: Text("  Buyer"))),
  ...
  Future<void> _displayTextInputDialog(BuildContext context, final data) async {
    // String? valueText;
    List<String> supList = [];
    DatabaseEvent event = await ref.child("buys").once();
    ...
    builder: (context) {
      ...
      return AlertDialog(
        title: Row(
          ...
          children: [
            Text(data == null ? 'Coir Output' : 'Coir Output Edit'),
          ],
        ),
        content: Row(
              ...
              if (formKey.currentState!.validate()) {
                ...
                try {
                  if (formKey.currentState!.validate() && (pay.text != "0" && weight.text != "0" )) {
                    ...
                    if(data == null) { 
                      await ref.child("coir_out").update({
                        now.toString().substring(0,19) : {
                          "by": globals.isAdmin,
                          "date": now.toString(),
                          "payment": pay.text,
                          "buyr": dropdownValue.toString(),
                          "weight": weight.text,
                        }
                      });
                    }
                    ...
                    await ref.child("account").child("coir").set(coir - int.parse(weight.text));
                    Navigator.pop(context);
                    pay.clear();
                    weight.clear();
                  }
